...
class ParserTest < Minitest::Test
  ...
  def test_parse_filter_adapter_v2_not_found_operator
    assert_raises ArelRest::Parser::OperatorNotFound do
      ArelRest::Parser.parse_filter_to_arel({
  ...
  def test_parse_filter_adapter_v2
    assert_equal ArelRest::Parser.parse_filter_to_arel({
      "and" => [
  ...
  def test_less_than_number_v2
    assert_equal ArelRest::Parser.parse_filter_to_arel({"and" => [{attribute: "row.colum1", operator: "lt", values: 202}]}).to_sql,
      "\"row\".\"colum1\" < 202"
  end

  def test_less_than_string_v2
    assert_equal ArelRest::Parser.parse_filter_to_arel({"and" => [{attribute: "row.colum1", operator: "lt", values: "'2024-01-01 14:10'"}]}).to_sql,
      "\"row\".\"colum1\" < '''2024-01-01 14:10'''"
  end

  def test_contains_v2
    assert_equal ArelRest::Parser.parse_filter_to_arel({"and" => [{attribute: "row.colum1", operator: "matches", values: "some things" }]}).to_sql,
      "\"row\".\"colum1\" LIKE '%some things%'"
  end

  def test_in_v2
    assert_equal ArelRest::Parser.parse_filter_to_arel({"and" => [{attribute: 'row.colum1', operator: 'in', values: [1,2,3]}]}).to_sql,
     "\"row\".\"colum1\" IN (1, 2, 3)"
  end

  def test_not_in_v2
    assert_equal ArelRest::Parser.parse_filter_to_arel({"and" => [{attribute: 'row.colum1', operator: 'not_in', values: [1,2,3]}]}).to_sql,
     "\"row\".\"colum1\" NOT IN (1, 2, 3)"
  end

  def test_lesser_equal_than_string_v2
    assert_equal ArelRest::Parser.parse_filter_to_arel({"and" => [{attribute: "row.colum1", operator: "lteq", values: "'2024-01-01 14:10'" }]}).to_sql,
      "\"row\".\"colum1\" <= '''2024-01-01 14:10'''"
  end

  def test_greater_than_number_v2
    assert_equal ArelRest::Parser.parse_filter_to_arel({"and" => [{attribute: "row.colum1", operator: "gt", values: "202"}]}).to_sql,
      "\"row\".\"colum1\" > '202'"
  end

  def test_greater_than_string_v2
    assert_equal ArelRest::Parser.parse_filter_to_arel({"and"=>[{attribute: "row.colum1", operator: "gt", values: "'2024-01-01 14:10'"}]}).to_sql,
      "\"row\".\"colum1\" > '''2024-01-01 14:10'''"
  end

  def test_greater_equal_than_string_v2
    assert_equal ArelRest::Parser.parse_filter_to_arel({"and"=>[{attribute: "row.colum1", operator: "gteq", values: "'2024-01-01 14:10'"}]}).to_sql,
      "\"row\".\"colum1\" >= '''2024-01-01 14:10'''"
  end
